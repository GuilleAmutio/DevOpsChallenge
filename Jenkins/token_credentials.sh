#!/bin/bash
token=$(curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "name=WindowsTokenFinal" -u admin:admin 10.0.0.2:9000/api/user_tokens/generate | jq -r '.token' | xargs)
COOKIEJAR="$(mktemp)"
